...
export default function Dashboard() {
  ...
  const [user, setUser] = useState({ name: 'Olana Kenea', email: 'Loading...' });

  // NEW: The Auth Guard State
  ...
  useEffect(() => {
    ...
    if (!token) {
      // If no token exists, redirect instantly. 
      // Notice we DO NOT set isAuthChecking to false here, so the screen stays blank.
      navigate('/login');
      return;
    }

    // 2. If token exists, load their profile data
    const storedUser = localStorage.getItem('sky_user');
    if (storedUser) {
      setUser(JSON.parse(storedUser));
    }

    // 3. Token is verified! Lift the gate and reveal the dashboard.
    ...
    // NEW: The Auth Guard
    // If we are still checking the token, render a completely blank background matching the theme.
    if (isAuthChecking) {
    ...
    return (
      <div className="flex h-screen bg-white dark:bg-gray-900 font-sans text-gray-800 dark:text-gray-200 relative transition-colors duration-300">
        ...
        <aside className="w-64 border-r border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex flex-col justify-between overflow-y-auto transition-colors duration-300">
          ...
          {/* User Profile Area (Updated with Dynamic User Data) */}
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 relative">
            ...
            >
              {/* Grab the first letter of their name dynamically */}
