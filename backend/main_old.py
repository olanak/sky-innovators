...
from segmentation import generate_segmentation_for_media, save_segmentation_frames
...
async def upload_drone_footage(
    ...
    try:
        ...
        if len(parsed_modules) > 0:
            ...
            # ── NEW: generate and save pixel segmentation masks ──────────────
            seg_frames = generate_segmentation_for_media(safe_filename, parsed_modules)
            save_segmentation_frames(new_media.id, seg_frames, db)
            # ─────────────────────────────────────────────────────────────────
...
# ── NEW: Segmentation endpoint — consumed by AnalysisReport.jsx canvas overlay
...
def get_segmentation_frames(
    ...
    Frontend AnalysisReport.jsx fetches this on mount and uses it to
    drive the canvas overlay for both images and videos.
        ...
        "frames": [
            {
                "timestamp_ms": 0,
                "segments": [
                    { "class": "forest", "mask_rle": [...], "width": 1280, "height": 720, "confidence": 0.94 },
                    ...
                ]
            },
            ...
    ...
    """
    # Security: verify media belongs to this user
    media_item = db.query(models.MediaFile).filter(
        models.MediaFile.id == media_id,
        models.MediaFile.owner_id == current_user.id
    ).first()
